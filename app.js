...
document.querySelectorAll(".nav-btn").forEach(b=>b.onclick=()=>{showView(b.dataset.view);closeMobileNav()});document.querySelectorAll("[data-view-go]").forEach(b=>b.onclick=()=>showView(b.dataset.viewGo));function showView(v){document.querySelectorAll(".view").forEach(x=>x.classList.toggle("active",x.id===v));document.querySelectorAll(".nav-btn").forEach(x=>x.classList.toggle("active",x.dataset.view===v))}
function openMobileNav(){$(".sidebar").classList.add("mobile-open");$("#sidebarBackdrop").classList.add("show")}
function closeMobileNav(){$(".sidebar").classList.remove("mobile-open");$("#sidebarBackdrop").classList.remove("show")}
$("#mobileNavToggle").onclick=()=>{$(".sidebar").classList.contains("mobile-open")?closeMobileNav():openMobileNav()};
$("#sidebarBackdrop").onclick=closeMobileNav;
window.matchMedia("(max-width:600px)").addEventListener?.("change",e=>{if(!e.matches)closeMobileNav()});
