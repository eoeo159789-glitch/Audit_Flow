...
function ensureCase(){if(!data.cases.length)data.cases.push({id:uid(),name:"我的第一個審計案件",number:"",agency:"",period:"",status:"進行中",events:[],issues:[],findings:[],evidence:[],procedures:[]});data.cases.forEach(c=>{["events","issues","findings","evidence","procedures","attachments"].forEach(k=>c[k]??=[])});activeCaseId||=data.cases[0].id}
...
function formWrap(f){return `<div class="form-grid">${f}</div><div class="form-actions"><button type="button" onclick="closeModal()">取消</button><button class="primary">儲存</button></div>`}
